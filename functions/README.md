...
`progression.js` is the server mirror of the mobile app's canonical
...
identical — `mobile/src/lib/__tests__/progressionParity.test.ts` imports this
file directly and fails CI if they drift. If you change a formula, change it
in both places and run `npm test` in `mobile/`.
